...
public class PatternObservateur extends junit.framework.TestCase {
    ...
    public void testNotify() {
        ...
        list = new ConcreteSubject();           // cr̩ation d'un "observ̩" constitu̩ d'une liste
        observer = new ConcreteObserver();      // cr̩ation d'un observateur
        ...
        list.insert("il fait beau, ce matin");  // modification de cette liste, l'observateur doit
                                                // (dervrait) ̻tre notifi̩

        // "v̩rification" :
        assertFalse(observer.senders().empty());                            // elle ne doit pas ̻tre vide,
        assertEquals(list, observer.senders().pop());                       // est-ce le bon ̩metteur ?
        assertEquals("il fait beau, ce matin", observer.arguments().pop()); // le param̬tre re̤u est-il correct ?
    }
